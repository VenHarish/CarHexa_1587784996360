...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findBy_Driver/:key", authorize([]), UserController.findBy_Driver);
  ...
  */
  ...
  /**
  * UserModel.findBy_Driver
  *   @description CRUD ACTION findBy_Driver
  *   @param Objectid key Id of model to search for
  *
  */
  findBy_Driver: async (req, res) => {
    try {
      const result = await UserModel.findBy_Driver(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
